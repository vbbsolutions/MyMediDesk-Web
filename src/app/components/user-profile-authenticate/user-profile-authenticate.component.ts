...
@Component({
  selector: 'app-user-profile-authenticate',
  templateUrl: './user-profile-authenticate.component.html',
  styleUrls: ['./user-profile-authenticate.component.css']
})
export class UserProfileAuthenticateComponent implements OnInit {
  ...
  authenticateFun() {
    ...
    if (this.emailData.Id && this.emailData.Email && this.emailData.NewMobileNumber && this.loginId) {
      ...
      this.userService.reAuthenticate({ data: emailUpdateDateData }, this.loginId).subscribe((reAuthenticateRes: any) => {
  ...
  resendOTP() {
    ...
    if (this.emailData.Email && this.loginId) {
      ...
      this.userService.authenticateAgain({ data: resendData }).subscribe((emailDataRes: any) => {
